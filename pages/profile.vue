        ...
        <b-tabs pills card vertical end class="text-left">
          ...
          <b-tab title="Личные данные"
            ><b-card-text>
              ...
              <div v-if="user" class="privat-data">
                <client-only>
                  <div class="mb-3">
                    <div class="privat-data__title font-weight-semibold">
                      Имя:
                    </div>
                    <b-input-group v-click-outside="hideName">
                      <template #append>
                        <b-input-group-text
                          v-if="!name.isEdit"
                          @click="name.isEdit = true"
                          >Изменить</b-input-group-text
                        >
                        <b-input-group-text
                          v-if="name.isEdit"
                          class="btn"
                          @click="updateName"
                          >Сохранить</b-input-group-text
                        >
                      </template>
                      <b-form-input
                        ref="name"
                        class="privat-data__input"
                        :value="user.name"
                        :disabled="!name.isEdit"
                      ></b-form-input>
                    </b-input-group>
                  </div>
                  <div class="mb-3">
                ...
                <div>
                  <b-button variant="outline-danger" @click="exitProfile"
                    >Выйти из профиля</b-button
                  >
                </div>
              </div>
            </b-card-text></b-tab
          ...
          <b-tab title="Мои заказы"
            ><b-card-text>
              ...
              <div
                v-for="order in ordersReverse"
                :key="order.id"
                class="history-order-wrapper"
              >
                <div class="history-order-item">
                  <b-badge
                    v-if="isStatusPayment(order, 'succeeded')"
                    variant="success"
                    >Заказ принят</b-badge
                  >
                  <!--                <b-badge v-else-if="isStatusPayment(order, 'pending')" variant="warning">Ожидание</b-badge>-->
                  <b-badge
                    v-else-if="isStatusPayment(order, 'canceled')"
                    variant="danger"
                    >Заказ отменён</b-badge
                  >
                  ...
                  <div class="order-info">
                    <div>
                      ...
                      <span class="order-info__method">{{
                        order.isSelfService ? "самовывоз" : "доставка"
                      }}</span>
                    </div>
                    <div class="order-totalSum">
                      {{ totalSum(order.cart) }} ₽
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    isStatusPayment(order, status) {
      ...
      return order.yandexPayment && order.yandexPayment.status === status;
    },
    totalSum(cart) {
    ...
    formatDateTime(time) {
      return new DateTime.fromSQL(time, { locale: "ru" }).toFormat(
        "dd MMMM yyyy HH:mm"
      );
    },
    updateName() {
