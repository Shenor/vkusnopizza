<template>
  <b-modal id="modal-registration" title="Регистрация">
    <template v-slot:default>
        ...
        <form style="max-width: 236px;" ref="form" @submit.stop.prevent="enter">
          ...
          <b-form-group
            ...
            <div v-if="$v.registr.email.$error">
              <div v-if="!$v.registr.email.minLength" class="small text-danger px-2">Минимум {{$v.registr.email.$params.minLength.min}} символа</div>
            </div>
          </b-form-group>
          <b-form-group
            label-for="phone-input">
      ...
      <div class="sub-title text-center mt-3">Уже есть аккаунт? <span class="btn-link-modal" @click="$bvModal.hide('modal-registration'), $bvModal.show('modal-login')">Войти</span></div>
    </template>
