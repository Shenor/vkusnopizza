<template>
  <div class="menu-stickly">
    <div
      class="container menu-nav justify-content-start pt-3 pb-3 overflow-mobile"
    >
      <!-- Menu navigation stickly -->
      <div class="menu-stickly__wrapper-logo">
        <NuxtLink to="/">
          <b-img
            class="menu-stickly__logo"
      ...
      <div
        ...
        class="cart-wrapper ml-auto mr-3 d-none d-md-flex d-xl-flex"
      >
        <client-only>
          <b-button
            class="btn cart-btn rounded-pill d-flex"
            size="md"
            variant="secondary"
          >
            Корзина
            <div v-if="allItemsCart" class="cart-btn__separate"></div>
  ...
  methods: {
    ...
    onScrolling() {
      if (document.documentElement.scrollTop > 20) {
        document
          .querySelector(".menu-stickly")
          .classList.add("menu-stickly--isActive");
      } else {
        document
          .querySelector(".menu-stickly")
          .classList.remove("menu-stickly--isActive");
      }
    },
    exit() {
